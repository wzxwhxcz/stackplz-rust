...
use super::point_arg::{PointArg, EBPF_SYS_ENTER, EBPF_SYS_EXIT, EBPF_SYS_ALL, EBPF_UPROBE_ENTER};
use crate::argtype::consts::{POINTER, INT, UINT, UINT64};
...
impl ParamConfig {
    ...
    pub fn get_point_arg(&self, arg_index: u32, point_type: u32) -> Result<PointArg> {
        ...
        let mut point_arg = match point_type {
            ...
            EBPF_UPROBE_ENTER => {
                PointArg::new_uprobe(&arg_name, POINTER, reg_index)
            }
        ...
        match self.format.as_str() {
            ...
            "inotify_flags" | "access_flags" | "mmap_flags" | "mremap_flags" | 
            "file_flags" | "prot_flags" | "fcntl_flags" | "statx_flags" | 
            "unlink_flags" | "socket_flags" | "perm_flags" | "msg_flags" => {
...
fn compile_read_op(read_op_str: &str, point_arg: &mut PointArg) -> Result<()> {
    use crate::argtype::{add_read_move_reg, opc_add_offset, opc_sub_offset, 
                          opc_add_reg, opc_sub_reg, opc_read_pointer, 
                          opc_move_pointer_value, opc_save_addr, get_op};
